...
! functions (Cross_product, Triple_product and Axial) do not possess
! simple
! generalizations to a generic number N of dimensions. The others, while
! admitting such N-dimensional generalizations, have not all been
! provided
! with such generic forms here at the time of writing, though some of
! these 
...
! Orthogonalized: Orthogonalized version of second vector rel. to first
! unit v.
...
! Axial:          Convert axial-vector <--> 2-form (antisymmetric
! matrix)
! Diag:           Diagnl of given matrix, or diagonal matrix of given
! elements
! Trace:          Trace of given matrix
! Identity:       Identity 3*3 matrix, or identity n*n matrix for a
! given n
! Sarea:          Spherical area subtended by three vectors, or by
! lat-lon
!                 increments forming a triangle or quadrilateral
! Huarea:         Spherical area subtended by right-angled spherical
! triangle
!   SUBROUTINE:
! Gram:           Right-handed orthogonal basis and rank, nrank. The
! first 
!                 nrank basis vectors span the column range of matrix
!                 given,
!                 OR  ("plain" version) simple unpivoted Gram-Schmidt of
!                 a
!                 square matrix.
!
! In addition, we include routines that relate to stereographic
! projections
! and some associated mobius transformation utilities, since these
! complex
! operations have a strong geometrical flavor.
!
! DIRECT DEPENDENCIES
! Libraries[their Modules]: pmat[pmat]
! Additional Modules      : pkind, pietc, pietc_s
!
!============================================================================
module pmat4
!============================================================================
...
c(1)=a(2)*b(3)-a(3)*b(2); c(2)=a(3)*b(1)-a(1)*b(3);
c(3)=a(1)*b(2)-a(2)*b(1)
...
!=============================================================================
uv12=u(1)*v(2)-u(2)*v(1); uv13=u(1)*v(3)-u(3)*v(1);
uv14=u(1)*v(4)-u(4)*v(1)
                          uv23=u(2)*v(3)-u(3)*v(2);
uv24=u(2)*v(4)-u(4)*v(2)
...
a(1)=(b(3,2)-b(2,3))*o2; a(2)=(b(1,3)-b(3,1))*o2;
a(3)=(b(2,1)-b(1,2))*o2
...
!=============================================================================
! Compute the area of the spherical triangle, {v1,v2,v3}, measured in
! the
! right-handed sense, by dropping a perpendicular to u0 on the longest
! side so
...
! Triangle that is not degenerate. Cyclically permute, so side 3 is
! longest:
...
! estimate even when the triangle is very small, provided the
! B-increment
...
cb=sqrt(u1-ssb)
! Construct 2D normalized right-handed basis vectors with xb pointing to
! B:
xb=x2b/sb
yb=(/-xb(2),xb(1)/)
...
!=============================================================================
! Compute the area of the spherical quadrilateral with a vertex at
! latitude
...
! diagonal making the B-increment is not disproportionately small
! compared to 
...
! matrix is singular, the "sign" of the determinant, detsign, is
! returned
...
!=============================================================================
! A "plain" (unpivoted) version of Gram-Schmidt, for square matrices
! only.
use pietc_s, only: u0
implicit none
...
!=============================================================================
! Without changing (tall) rectangular input matrix a, perform pivoted
! gram-
! Schmidt operations to orthogonalize the rows, until rows that remain
! become
! negligible. Record the pivoting sequence in ipiv, and the
! row-normalization
! in tt(j,j) and the row-orthogonalization in tt(i,j), for i>j. Note
! that
! tt(i,j)=0 for i<j (tt is truncated lower triangular). The
! orthonormalized
! rows are returned in square array b, which is complete even when the
! effective rank < n. 
! The recorded row operations can be repeated on independent column
! vectors
! through the use of subroutine ROWOPS (in this module).
! It is recommended to rescale the original matrix A via a call to
! CORRAL
! (in this module) because the negligibility criterion depends upon an
! "epsilon" value that is fixed (10**(-13)) and assumes elements of a
! are
! never too different in magnitude from unity, unless they are actually
! zero.
!=============================================================================
use pietc, only: u0,u1
implicit none
...
! At this stage, all rows less than ii are already orthonormalized and
! are
...
! rows and pivot the largest of them into position ii:
   ...
   if(maxp<nepss)then !<- End of gram process; clean up and return
      b=u0
      b(1:ii-1,:)=aa(1:ii-1,:)
! fill the remaining rows, ii:n, of b with remaining orthonormal rows at
! random
      do iii=ii,n
...
!=============================================================================
! Apply the row-operations, implied by ipiv and tt returned by rowgram,
! to
...
!=============================================================================
! Find positive diagonals D and E and a Lagrange multiplier F that
! minimize
...
! Once a solution for D and E is found, return their exponentials, d and
! e,
! together with the rescaled matrix aa such that a = d.aa.e when d and e
! are
...
! Exponentiate the parts that become final scaling diagnonal matrices d
! and e:
do i=1,m
   d(i)=exp(h(i))
enddo
...
!=============================================================================
! Assuming that given orth33 is a 3*3 proper rotation matrix, derive an
! axial
! 3-vector, ax3, such that orth33 is implied by ax3 when the latter is
! interpreted as encoding a rotation (as in subroutine axtorot). Note
! that
! such ax3 are not unique -- adding any multiple of 2*pi times the
! parallel
...
! ax33 equivalent to the axial vector ax3 is exponentiated to obtain
! orth33.
!=============================================================================
implicit none
...
integer(spi),dimension(1):: ii
!==============================================================================
! construct the orthogonal matrix, t1, whose third row is the rotation
! axis
! of rot:
t1=rot; do i=1,3; t1(i,i)=t1(i,i)-1; u1(i)=dot_product(t1(i,:),t1(i,:));
enddo
...
! Hence deduce coefficients (in the form of a real 4-vector) of one of
! the two
...
!=============================================================================
! Multiply quaternions, a*b, assuming operation performed from right to
! left
!=============================================================================
implicit none
...
do i=1,n
!detb=u0; do i=1,n; detb=detb+a(i,i); enddo; detb=exp(detb)
detb=u0; do j=1,n; detb=detb+a(j,i); enddo; detb=exp(detb)
...
!=============================================================================
! Utility code for various Mobius transformations. If aa1,bb1,cc1,dd1
! are
...
! aa2 etc to zv, is equivalent to a single mapping zz-->zv by the
! transformatn
...
!=============================================================================
! The convention adopted for the complex derivative is that, for a
! complex
! infinitesimal map displacement, delta_z, the corresponding
! infinitesimal
...
! THE DERIVATIVE FOR THE IDEAL POINT AT INFINITY HAS NOT BEEN CODED
! YET!!!
!=============================================================================
implicit none
complex(dpc),             intent(IN ):: z
...
! with aa*dd-bb*cc=1, for a standard (north-)polar stereographic
! transformation
! that takes cartesian point, xc0 to the north pole, xc1 to
! (lat=0,lon=0),
...
! corresponding infz will be set "true" (and the z value itself not
! used).
...
! the projection pole (i.e., the South Pole for a north polar
! stereographic).
!============================================================================
implicit none
...
! Infz is .TRUE. only when z is at complex infinity; likewise infw and
! w.
! Infz is .TRUE. only when z is at complex infinity; likewise infw and
! w.
...
!=============================================================================
! Perform a complex Mobius transformation from cartesian vz to cartesian
! vw
! where the transformation coefficients are the standard aa,bb,cc,dd.
!=============================================================================
implicit none
