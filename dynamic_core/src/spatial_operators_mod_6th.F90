      ...
      do iPatch = ifs, ife
        do j = jts, jte
          do i = its, ite
            ...
            !vorticity(i,j,iPatch) = 0.5*( stat%v(ip1,j,iPatch) - stat%v(im1,j,iPatch) - sign(1.,stat%uc(i,j,iPatch))*(stat%v(ip1,j,iPatch)+stat%v(im1,j,iPatch)-2.*stat%v(i,j,iPatch)) )/dx &
            !                      - 0.5*( stat%u(i,jp1,iPatch) - stat%u(i,jm1,iPatch) - sign(1.,stat%vc(i,j,iPatch))*(stat%u(i,jp1,iPatch)+stat%u(i,jm1,iPatch)-2.*stat%u(i,j,iPatch)) )/dy &
