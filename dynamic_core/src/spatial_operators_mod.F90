      ...
      do iPatch = ifs, ife
        ...
        do j = jts, jte
          do i = its-1, ite
            ...
            eigenvalue_x(1) = uBdy - sqrt( 0.5 * ( mesh%matrixIG(1,1,i+1,j,iPatch) + mesh%matrixIG(1,1,i,j,iPatch) ) * phitBdy )
            eigenvalue_x(2) = uBdy + sqrt( 0.5 * ( mesh%matrixIG(1,1,i+1,j,iPatch) + mesh%matrixIG(1,1,i,j,iPatch) ) * phitBdy )
      ...
      do iPatch = ifs, ife
        ...
        do j = jts-1, jte
          do i = its, ite
            ...
            eigenvalue_y(1) = vBdy - sqrt( 0.5 * ( mesh%matrixIG(2,2,i,j+1,iPatch) + mesh%matrixIG(2,2,i,j,iPatch) ) * phitBdy )
            eigenvalue_y(2) = vBdy + sqrt( 0.5 * ( mesh%matrixIG(2,2,i,j+1,iPatch) + mesh%matrixIG(2,2,i,j,iPatch) ) * phitBdy )
